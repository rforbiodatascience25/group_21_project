...
file.rename(from = here("R/03_augmented.html"), 
            to = here("results/03_augmented.html"))
```
...
```{r}
png_results <- list.files(path = here("results"), pattern = ".png")

file_copied <- png_results |>
  map(~file.copy(from = paste0(here("results/"), .x),
                 to = paste0(here("doc/images/"), .x),
                 overwrite = TRUE))
```
