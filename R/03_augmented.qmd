...
## Augment metadata, normalize counts and join tables
...
augmented_metadata <- GEP_score_calculation(quant_counts, 
                                            metadata_clean)

augmented_metadata <- augmented_metadata |>
  mutate(TLS_status_bin = if_else(TLS_status == "control", 
                                  0, 1))
...
PCA_ready_table <- joined_data |>
  ...
  pivot_wider(names_from = GeneFeature,
              values_from = CPM) |>
  inner_join(augmented_metadata, 
             by = "Sample_names")

# Saving final table
write_tsv(joined_data, 
          file = "../data/03_augmented_table.tsv")

write_tsv(PCA_ready_table, 
          file = "../data/03_augmented_table_PCA.tsv")

write_tsv(augmented_metadata,
          file = "../data/03_augmented_metadata.tsv")
```
