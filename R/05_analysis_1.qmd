...
# joined CPM + metadata
cpm_joined <- cpm_long |>
  left_join(augmented_metadata, by = "Sample_names")
```

```{r}
#  MODEL 1: CPM ~ TLS_status
nested_data <- cpm_joined |>
  group_by(GeneFeature) |>
  nest()

nested_data <- cpm_nested |>
  mutate(
    model = map(data, ~lm(CPM ~ TLS_status, data = .x)),
    tidy_model = map(model, ~tidy(.x, conf.int = TRUE))
  )

gene_results <- nested_data |>
  unnest(tidy_model) |>
  filter(term == "TLS_statusTLS") |>
...
gep_model <- lm(GEP_score ~ TLS_status, data = augmented_metadata)
...
forest_plot <- gene_results |>
  ...
  labs(
    ...
    subtitle = "Forest plot of statistically significant genes (FDR < 0.05)",
