...
GEP_score_vs_TLS_status <- ggplot(
  augmented_metadata, 
  aes(x = factor(TLS_status_bin), y = GEP_score, fill = factor(TLS_status_bin))
) +
  ...
  scale_fill_manual(values = c("0" = "lightsteelblue2", "1" = "indianred3")) +
  scale_color_manual(values = c("0" = "lightsteelblue4", "1" = "indianred4")) +
  labs(
    title = paste0(
      "GEP Score by TLS Status (LM p = ", signif(tls_p, 3), ")"
    ),
    x = "TLS Status",
    y = "GEP Score"
  ) +
