...
```{r}
siginifcant_genes_overlap <- ggplot(overlap_df, aes(x = Overlap, y = Gene, color = Overlap)) +
  geom_point(size = 4) +
  scale_color_manual(values = c("No" = "lightsteelblue4", "Yes" = "indianred3")) +
  labs(
  ...
  panel.grid.minor.y = element_line(color = "grey70", size = 0.3),
  panel.grid.minor.x = element_line(color = "grey70", size = 0.3)
  )


siginifcant_genes_overlap
ggsave(
  filename = "siginifcant_genes_overlap.png",
  path = here("results"),
  dpi = 300
)
